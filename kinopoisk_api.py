...
def fetch_film_data(films_data):
    ...
    for row in films.itertuples(index=False):
        encoded_string = urllib.parse.quote(str(row.title))
        text_full = text_begin + encoded_string + text_token + text_year + str(row.year)
        response = requests.get(text_full)
        items = json.loads(response.text)
        # нужно сделать проверку по году +-1/2 eyear, ибо многое фильмы различаются
        if items['pages'] == 0:
            df_err = pd.DataFrame([[0, row.title, 0, 0]], columns=['url', 'title', 'rating', 'year'])
